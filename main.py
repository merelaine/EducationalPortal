...
if __name__ == '__main__':
    ...
    xml_repo = XMLRepository("data.xml")
    teacher_repo = TeacherRepository()
    course_repo = CourseRepository()
    topic_repo = TopicRepository()
    task_repo = TaskRepository()
    studtask_repo = StudTaskRepository()
    xml_repo.read_data(Student=stud_repo,Teacher=teacher_repo,Course=course_repo,Topic=topic_repo,Task=task_repo,StudentTask=studtask_repo)
    print(stud_repo.get(3).name)
    ...
    studtask = Student_Task(id=1,student=1,task=1,score=5,time=300,date='10-10-2000')
    ...
    xml_repo.read_data(stud_repo)
    xml_repo.read_data(course_repo)
    xml_repo.read_data(teacher_repo)
    xml_repo.read_data(topic_repo)
    xml_repo.read_data(task_repo)
    xml_repo.read_data(studtask_repo)


    xml_repo.save_to_xml()
